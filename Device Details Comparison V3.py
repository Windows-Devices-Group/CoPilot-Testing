...
def get_comp_device_details(user_input, df1):
    ...
    try:
        ...
        ASP = "NA"
    net_sentiment = get_net_sentiment(dev)
...
def generate_device_details(device_input):
    ...
    return device_name, img_link, net_Sentiment, total_sales, asp, high_specs, sale, star_rating_html, comp_devices
    ...
    # Chat history state management
        ...
        if user_input:
            ...
            if not st.session_state['chat_history']:
                device_name, img_link, net_Sentiment, total_sales, asp, high_specs, sale, star_rating_html, comp_devices = generate_device_details(inp)
#                 summ = get_detailed_summary(inp)
                summ = "Summary Placeholder"
                st.session_state['chat_history'].append((inp, device_name, img_link, net_Sentiment, total_sales, asp, high_specs, sale, star_rating_html, comp_devices, summ))
...
#                 summ = get_detailed_summary(inp)
            ...
            i = 0
            for device in comp_devices['SERIES']:
                checkbox_state[i] = st.checkbox(device)
                i+=1
            i = 0
            for device in comp_devices['SERIES']:
                if checkbox_state[i]:
                    st.session_state['selected_device_comparison'] = device
                    st.write(f"You have selected device {device}")
                    break
                i+=1
                st.session_state['selected_device_comparison'] = None
            if st.session_state['selected_device_comparison']:
                st.write(f"Printing details for {st.session_state['selected_device_comparison']} device")
